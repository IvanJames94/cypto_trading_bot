from trality.indicator import rsi, ppo, ema, crossany
import numpy as np 

AUTHOR = "Ivan James Ostojić"

# TRADING PAIRS
SYMBOL_GROUP_1 = ["LUNABUSD","AVAXBUSD","ATOMBUSD","MATICBUSD","ONEBUSD"]
#TREND_STATUS
...
def initialize(state):
    ...
    '''
    25.08-08.11.2021, BACKTEST RESULT
    Total Return (%) 79.50%
    symbol    win/total | average_profit | average loss 
    AVAXBUSD     9/9    | 4.17053        | 0.00000      
    KEEPBUSD    12/14   | 37.28082       | -18.10838   
    ONEBUSD     24/27   | 13.46876       | -17.03190   
    FTMBUSD     18/24   | 25.48463       | -3.45122    
    LUNABUSD     6/8    | 3.13744        | -6.06412     
    DOTBUSD      9/11   | 3.24709        | -11.67195    
    MATICBUSD    3/7    | 7.09002        | -4.22072     
    ATOMBUSD     7/12   | 3.56141        | -10.81743   
    '''
    ...
    '''
    state.last_run_pnl["AVAXBUSD"] =  [ 9,9 ,4.17053 ,0.00000  ]   
    state.last_run_pnl["KEEPBUSD"] =  [12,14,37.28082,-18.10838]  
    state.last_run_pnl["ONEBUSD"] =  [24,27,13.46876,-17.03190]  
    state.last_run_pnl["FTMBUSD"] =  [18,24,25.48463,-3.45122 ]  
    state.last_run_pnl["LUNABUSD"] =  [ 6,8 ,3.13744 ,-6.06412 ]   
    state.last_run_pnl["DOTBUSD"] =  [ 9,11,3.24709 ,-11.67195]   
    state.last_run_pnl["MATICBUSD"] =  [ 3,7 ,7.09002 ,-4.22072 ]   
    state.last_run_pnl["ATOMBUSD"] =  [ 7,12,3.56141 ,-10.81743]
    '''
    for symbol in SYMBOL_GROUP_1:
...
def compute_trend(state, data, trend_long = TREND_LONG, trend_short = TREND_SHORT, trend_ema = TREND_EMA):
    ...
    ppo_crossany = crossany(np.vstack((ppOscillator[-12:], ppo_signal[-12:])))[0]

    if sum(ppo_crossany[-4:]) == 1:
    ...
    #DOWN MOMENTUM
        ...
        if abs(ppo_signal[-1] - ppOscillator[-1]) > 2.5 or sum(ppo_crossany) == 0:
...
def handler_main(state,data):
    ...
    if trend >= SIDEWAYS_TREND and trend_momentum >= UP and recent_momentum_cross and not gold_up:
        no_sell = True
    ...
    #BUY CONDITIONS
    if rsi_ema[-1] < rsi_low and rsi1 > rsi_ema[-1]:
...
def get_buy_value(state, symbol):
    portfolio = query_portfolio()
    balance_free = float(query_balance_free("BUSD"))
    ...
    buy_value = portfolio_value * min(max_allocation/4, max_allocation - asset_allocation)
...
def get_sell_value(state, symbol):
    ...
    sell_value = portfolio_value * min((max_allocation/4), asset_allocation) * -1

    if max_allocation < asset_allocation:
        sell_value = portfolio_value * max((asset_allocation - max_allocation, asset_allocation/4)) * -1
